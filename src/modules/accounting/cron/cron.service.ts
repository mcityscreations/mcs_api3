...
export class AccountingCronService {
	...
	async sendInvoices():Promise<void> {
		...
		if (now < serviceStartDate) {
			this.logger.log(
				'Service not started yet. First execution will be on November 2nd 2026.',
			);
		} else {
			const startOfPeriod = this.dateService.dateOnlyFormatter(
			...
			try {
				const response = await axios.post(
					'http://localhost:3000/accounting/invoices/send',
					{
						startOfPeriod,
						endOfPeriod,
					},
				...
				this.logger.error('Error sending invoices', errorMessage);
			}
		}
	}
}
